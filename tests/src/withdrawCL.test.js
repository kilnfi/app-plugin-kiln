...
nano_models.forEach(function(model) {
    test('[Nano ' + model.letter + '] Withdraw CL', zemu(model, async (sim, eth) => {
        ...
        const {data} = await contract.populateTransaction.withdrawCLFee(validatorAddress);
        ...
        await sim.navigateAndCompareSnapshots('.', model.name + '_withdrawCL', [right_clicks, 0]);
